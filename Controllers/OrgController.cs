...
public class OrgController(AppDbContext db) : Controller
{
    ...
    {
        // Bỏ query filter để liệt kê MỌI tổ chức (Org không bị filter, nhưng để rõ ràng).
    ...
    public async Task<IActionResult> Create(string name)
    {
        ...
        var org = new Org { Name = name.Trim(), ApiKey = "csk_" + Guid.NewGuid().ToString("N") };
        db.Orgs.Add(org);
        await db.SaveChangesAsync();
        SetOrgCookies(org.ApiKey, org.Name);
        TempData["Success"] = $"Đã tạo & chuyển sang tổ chức \"{org.Name}\". Dữ liệu CSKH cô lập riêng.";
    ...
    public IActionResult Reset()
    {
        ...
        TempData["Success"] = "Đã về tổ chức mặc định (Demo CSKH).";
        return RedirectToAction("Index", "Home");
    }
